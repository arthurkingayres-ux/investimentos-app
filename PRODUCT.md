...
**Nav model: bottom tab bar fixa, 5 destinos top-level.** Indicator 2px no topo da tab ativa; safe-area iOS honored. Tab bar persiste em telas de push (`#ativo/:ticker`, `#/raiox/chart`, `#/proventos/dy`); some apenas na PIN screen.

Tabs (top-level):
- **Raio-X** (`#`) — hero patrimônio Monument + bloco "Últimos 7 dias" (delta Δ + decomposição contábil Aportes/Proventos/Mercado + **"Variação no mercado"**: top 3 altas + 3 baixas por impacto em R$, que explicam *quais ativos* moveram a linha Mercado — 7a.J.2 + listas de compras/vendas/proventos da semana) + bloco "Último aporte". Glance airy de 3 blocos: estado (quanto), delta (o que mudou em 7d), evento (última compra). Sem charts no home — sparkline/chips removidos no raio-x enxuto (7a.I pós-fechamento); chart histórico full vive em `#/raiox/chart` (push). Abaixo do "Último aporte", um card discreto **"Relatório do mês"** (7a.Q.3) abre o briefing mensal do assessor em push (`#/raiox/relatorio`); **empty-safe** — o card some quando não há relatório publicado. Logo abaixo dele, no mesmo idioma visual, um card **"Projeção até os 65"** (7a.AV.2, reescrito na 7a.AW.2) abre a projeção patrimonial em push (`#/raiox/projecao`) e diz **"pelo seu histórico, entre R$ A e R$ B"** (as medianas das duas leituras do histórico); com uma delas ausente, diz um valor só e nomeia a medida. Também **empty-safe**: some quando o payload não traz `projecao` (`null`, ou schema anterior à v2.28).
...
- **Proventos** (`#proventos`) — abre no eyebrow "Proventos" + toggle Anual/Mensal; sem poster de total no topo (7a.S.7b). O card do chart ECharts traz uma **linha-razão** ("Total · 2021–2026" anual / "Total · 12 meses" mensal) que é sempre a soma exata das barras exibidas, trocando com o toggle — drilldown mês × ativo (7a.E.18) intacto. Abaixo, uma **chamada** de Dividend Yield (idioma do card "Relatório do mês") mostra o DY trailing-12m da carteira e abre a tela dedicada `#/proventos/dy`.
- **Aportar** (`#aportar`) — executor de política: valor → cotas (cap-5 picks; sobra quando não fecha o valor inteiro; seletor Tudo/Brasil/EUA restringe o plano à categoria-mercado, 7a.AU).
...
- `#/raiox/relatorio` · `#/raiox/relatorio/:mes` — **Relatório do mês** (7a.Q.3, read-only): briefing mensal interpretado do assessor. Scroll único curado de 9 seções (prestação de contas · leitura do mês · como você foi · funcionando · **NÃO funcionando** em destaque · renda · alinhamento · radar · evidências) + mini-cards do dossiê (`.kpi`) + selos de veredito por ticker (Tese intacta / Sob pressão / Deteriorando — forma+texto+cor) + citações com link. Seletor de mês ▾ (arquivo) decifra **sob demanda**. Default abre o último mês. Consome payloads cifrados **separados** (`relatorios_index.json.enc` no boot + `relatorio_<mes>.json.enc` lazy), fora do `portfolio.json` (que segue v2.22). Régua "só sinaliza" — radar + gatilho, nunca comprar/vender. Acessado pelo card na home Raio-X.
- `#/raiox/projecao` — **Projeção patrimonial até os 65, como ensaio** (7a.AW.2, read-only; substitui a tela da 7a.AV.2), forma refeita na 7a.AX, sem gráfico ECharts. Abre com a frase-resposta "Pelo seu histórico, aos 65 você teria entre R$ A e R$ B", em reais de hoje, e segue em **seis capítulos**, cada um com número-tema, um parágrafo e "ver a conta" (a derivação, fechada por padrão): 1 de onde você parte · 2 quanto você aporta · 3 quanto a carteira rende, com as barras das quatro taxas, uma subseção por cenário e o grifo "por que as duas medidas do seu histórico discordam" · 4 o que isso dá aos 65 (gráfico de faixas das quatro leituras com os valores escritos, a largura das faixas e de onde vem o valor) · 5 o que mexe o ponteiro (lista por alavanca, uma linha por âncora) · 6 o que esta conta não inclui. **Quatro cenários, sempre com estes nomes e nesta ordem:** Retorno da carteira e Retorno do seu dinheiro (as duas âncoras, lidas do histórico) · Mercado, média das classes e Mercado, com rebalanceamento (comparação). **O aporte é sempre a média dos últimos 12 meses**: sem estimativa do usuário e sem marco de formação. Monte Carlo recalculado toda noite pelo pipeline, nunca ajustável na tela; mudar premissa é editar `config/projecao.yaml` pela skill `/plano-aposentadoria`. **Estados:** âncora ausente sai das barras de taxa, do gráfico de faixas, da decomposição e do ponteiro, e a frase-resposta diz um valor só; `projecao: null` ou payload anterior à v2.28, o card some e a rota diz "Projeção indisponível hoje". Consome o mesmo `portfolio.json` (**schema v2.28**), campo `projecao`, sem payload cifrado próprio.
- `#/dossie/<TICKER>` — **Dossiê de empresa** (7a.R.3.b, read-only): a memória viva da posição — cabeçalho + mini-mapa do arco de vereditos + **tese** (com o `quebra_se` no único grifo da tela) + linha do tempo append-only das leituras datadas, cada uma com suas fontes. Não é um segundo `#ativo`: zero preço, zero DY, zero KPI vivo. Desde a 7a.AD.2, o bloco da tese fecha com o selo derivado **"esta tese pede revisão"**, que aparece quando uma leitura confrontou o `quebra_se` e o **disparou** depois da última revisão da tese — e some sozinho quando o Dr. Arthur a revisa. Alcançado de **duas** telas: a linha `.dossie-entrada` ao fim do `#ativo` ("por que eu tenho isso?") e o ticker linkado no radar/prestação do Relatório ("sob pressão contra o quê?"). Consome `dossies_index.json.enc` + um `.enc` por ticker (nomes por HMAC desde a 7a.W), fora do `portfolio.json`.
- `#/proventos/dy` — **Dividend Yield dedicado** (7a.S.7b): poster mono com o DY da carteira (trailing-12m) + 3 linhas por classe (FII / Ação BR / EUA·USD) com barra proporcional ao maior yield + **pódio de campeões** (top-3 yielders por categoria, toque numa linha troca qual pódio aparece). Empty-safe (categoria sem campeões suficientes não quebra a tela). Acessado pela chamada de Dividend Yield em `#proventos`.
