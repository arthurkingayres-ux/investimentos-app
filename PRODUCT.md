...
- **Alocação** (`#alocacao`) — segmented `Atual · Alvo`. Cada vista (Atual/Alvo) fica sob um cabeçalho de seção colapsável que **começa fechado** (7a.E.30): a tela abre enxuta (só tabs + cabeçalhos), o conteúdo é revelado ao tocar o cabeçalho. Os dois estados são independentes, não-persistentes (resetam no reload) e preservados ao trocar de aba. Atual: classes (BR/EUA/FII/Cripto/Renda Fixa BR) com trilha + drilldown por ticker. Alvo (reescrita em 7a.E.23): política híbrida v3, cada categoria como card sempre expandido dentro da seção (sem accordion por-card) com identidade `--cat-*` em stripe à esquerda + número-poster mono do alvo% + trilha categoria com marker preto. Cada card mostra duas **cestas**: `Cesta passiva` (ETFs broad-index com peso fixo, ex.: BOVA11 60 % + SMAL11 40 % dentro da cesta passiva de Ações BR) + `Cesta de picks` (seleção individual equal-weight, ex.: 11 nomes BR a 9,09 % cada). Cada ativo tem mini-bar `atual / alvo intra` + delta pill mono direcional (`↑ −5,98 pp` = precisa aportar; `↓ +4,30 pp` = acima do alvo). Backend mantém `bucket.tipo: "passive" | "picks"` em `config/alocacao.yaml` (`schema_version: 3`); o rebrand para "Cesta passiva" / "Cesta de picks" é só de apresentação.
