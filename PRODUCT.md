...
Tabs (top-level):
- **Raio-X** (`#`) — hero patrimônio Monument + bloco "Últimos 7 dias" (delta Δ + decomposição contábil Aportes/Proventos/Mercado + **"Variação no mercado"**: top 3 altas + 3 baixas por impacto em R$, que explicam *quais ativos* moveram a linha Mercado — 7a.J.2 + listas de compras/vendas/proventos da semana) + bloco "Último aporte". Glance airy de 3 blocos: estado (quanto), delta (o que mudou em 7d), evento (última compra). Sem charts no home — sparkline/chips removidos no raio-x enxuto (7a.I pós-fechamento); chart histórico full vive em `#/raiox/chart` (push). Abaixo do "Último aporte", um card discreto **"Relatório do mês"** (7a.Q.3) abre o briefing mensal do assessor em push (`#/raiox/relatorio`); **empty-safe** — o card some quando não há relatório publicado. Logo abaixo dele, no mesmo idioma visual, um card **"Projeção até os 65"** (7a.AV.2) abre a projeção patrimonial em push (`#/raiox/projecao`); também **empty-safe** — some quando o payload não traz `projecao` (schema anterior à v2.27, ou premissas ausentes/inválidas).
...
- `#/raiox/projecao` — **Projeção patrimonial até os 65** (7a.AV.2, read-only): responde "quanto vou ter aos 65?" em uma linha (faixa p10–p90 + valor central, em reais de hoje), seguida do gráfico em leque (Monte Carlo, milhares de trajetórias com semente fixa, recalculado toda noite pelo pipeline — nunca ajustável na tela; mudar premissa é editar `config/projecao.yaml` pela skill `/plano-aposentadoria`) com faixas p10–p90/p25–p75, mediana e, quando a fonte declara, a curva histórica tracejada ("se repetir seus últimos N anos") e um marco anotado (ex. fim da formação cirúrgica). Abaixo: **"E se"** (sensibilidade contínua a aporte/retorno/prazo) · **"Cenários hipotéticos"** (estresse nomeado: recessão, crise setorial etc.) · **"Premissas"** (aporte mensal antes/depois do marco + peso-alvo/retorno real/volatilidade por classe de ativo, cada linha citando a fonte). Consome o mesmo `portfolio.json` (schema v2.27), campo `projecao` — sem payload cifrado próprio. Acessado pelo card na home Raio-X, logo abaixo de "Relatório do mês"; **empty-safe** (some sem o campo `projecao`).
