...
- `#/raiox/projecao` — **Projeção patrimonial até os 65, como ensaio** (7a.AW.2, read-only; substitui a tela da 7a.AV.2). Abre com a frase-resposta "Pelo seu histórico, aos 65 você teria entre R$ A e R$ B", em reais de hoje, e segue em **sete capítulos**, cada um com número-tema, um parágrafo e "ver a conta" (a derivação, fechada por padrão): 1 de onde você parte · 2 quanto você aporta · 3 quanto a carteira rende, com a régua das quatro taxas, uma subseção por cenário e o grifo "por que as duas medidas do seu histórico discordam" · 4 o que isso dá aos 65 (mapa das quatro medianas, tabela taxa → valor, decomposição de cada âncora) · 5 a incerteza (um leque por âncora, mesma escala) · 6 o que mexe o ponteiro (sensibilidades e cenários hipotéticos, uma coluna por âncora) · 7 o que esta conta não inclui. **Quatro cenários, sempre com estes nomes e nesta ordem:** Retorno da carteira e Retorno do seu dinheiro (as duas âncoras, lidas do histórico) · Mercado, média das classes e Mercado, com rebalanceamento (comparação). **O aporte é sempre a média dos últimos 12 meses**: sem estimativa do usuário e sem marco de formação. Monte Carlo recalculado toda noite pelo pipeline, nunca ajustável na tela; mudar premissa é editar `config/projecao.yaml` pela skill `/plano-aposentadoria`. **Estados:** âncora ausente sai da régua, do mapa, dos leques e do ponteiro, e a frase-resposta diz um valor só; `projecao: null` ou payload anterior à v2.28, o card some e a rota diz "Projeção indisponível hoje". Consome o mesmo `portfolio.json` (**schema v2.28**), campo `projecao`, sem payload cifrado próprio.
