...
17. **Gradient fill em series area** de chart ECharts. Linhas sólidas (ou dashed para aporte cumulativo / benchmark) sem fill decorativo. **EXCEÇÃO CONSAGRADA (7a.AV.2) — faixas de incerteza do leque de projeção:** `hidratarProjecao()` (`#/raiox/projecao`) empilha duas séries de área com `areaStyle` **plano** (cor sólida `dc.tokens.g700`, sem `graphic`/gradiente), opacidade fixa 0,12 (p10–p90) e 0,22 (p25–p75) — a área **é** o dado (a incerteza da simulação Monte Carlo), não decoração de linha, no mesmo espírito do markPoint "símbolo no último ponto" (item 20): instrumento de leitura, não estética. Continua banido gradiente em qualquer OUTRA série de área do app (histórico, benchmarks) — a exceção é nomeada e escopada ao leque, não geral a "quando a área carrega dado".
...
/* Componentes */    .eyebrow (11px/800/.2em/upper/--faint; 7a.S.4 = voz única de abertura de tela) .eyebrow--accent (cor --accent, push screens)
                     ...
                     .proj-resposta/.proj-chart/.proj-sens/.proj-estresse/.proj-premissas/.proj-fonte (Projeção patrimonial #/raiox/projecao, 7a.AV.2 — ver seção dedicada abaixo); .proj-card-home agrupado com .rel-card-home (mesmo idioma, seletor próprio)
...
- **Gráfico em leque, zero dataZoom.** `.proj-chart` monta o leque de incerteza (ver a exceção nomeada ao anti-pattern #17, acima): faixas p10–p90 e p25–p75 empilhadas em preenchimento plano, mediana em linha sólida com `criarMarkPointUltimo` (7a.S.3) no último ponto, curva histórica tracejada quando a fonte declara `historico` ("se repetir seus últimos N anos"), e `markLine` silenciosa e tracejada no índice do marco (omitida de propósito quando o marco cai fora do horizonte projetado, ou seja, formação já encerrada ou além do último ponto: não há o que marcar) (ex. fim da formação cirúrgica) rotulada inline — nunca por data, por **índice** no eixo de categoria, porque rótulos de idade podem se repetir entre trajetórias sintéticas de teste. Sem dataZoom: o horizonte inteiro (hoje → 65) é a leitura pretendida, ao contrário do `#rentabilidade` (7a.S.6), onde o zoom é o instrumento.
