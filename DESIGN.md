...
17. **Gradient fill em series area** de chart ECharts. Linhas sólidas (ou dashed para aporte cumulativo / benchmark) sem fill decorativo. **EXCEÇÃO CONSAGRADA (7a.AV.2, migrada na 7a.AW.2) — faixas de incerteza dos leques de projeção:** nasceu no gráfico único da 7a.AV.2 e, desde a 7a.AW.2, vive nos **leques em pequenos múltiplos** do capítulo 5 (um por âncora, `_opcaoLequeProj`, `#chart-projecao-leque-<id>`). Cada leque empilha duas séries de área com `areaStyle` **plano** (cor sólida da âncora: `dc.tokens.ink` no retorno da carteira, `dc.tokens.g700` no do seu dinheiro; sem `graphic`/gradiente), opacidade fixa 0,12 (p10–p90) e 0,22 (p25–p75); o gráfico-mapa do capítulo 4 **não** tem área nenhuma — a área **é** o dado (a incerteza da simulação Monte Carlo), não decoração de linha, no mesmo espírito do markPoint "símbolo no último ponto" (item 20): instrumento de leitura, não estética. Continua banido gradiente em qualquer OUTRA série de área do app (histórico, benchmarks) — a exceção é nomeada e escopada ao leque, não geral a "quando a área carrega dado".
...
/* Componentes */    .eyebrow (11px/800/.2em/upper/--faint; 7a.S.4 = voz única de abertura de tela) .eyebrow--accent (cor --accent, push screens)
                     ...
                     .proj-ensaio/.proj-cap/.proj-regua/.proj-mapa/.proj-decomp/.proj-leque/.proj-tabela/.proj-conta (Projeção como ensaio #/raiox/projecao, 7a.AW.2 — ver seção dedicada abaixo; a da 7a.AV.2 ficou como histórico); .proj-card-home agrupado com .rel-card-home (mesmo idioma, seletor próprio)
...
- **Limpeza no lock.** `echartsProj`/`resizeObserverProj` seguem o mesmo padrão dos outros três gráficos ECharts do app (`_descartarProj()`, chamado no lock e a cada re-entrada na rota): dispose incondicional, nunca estado vivo sobrevivendo à saída da tela.

### Projeção — ensaio (7a.AW)
...
- **Capítulo (`.proj-cap`, 7 na tela).** Gramática única: ordinal (`.proj-cap__ordinal`, pequeno, `--accent`, tipografia de título e **nunca** `--num-*`) → título (`.proj-cap__titulo`, 1,25rem/700) → número-tema (`.proj-cap__tema`, mono, `--num-lg`, tabular) → texto (`.proj-cap__texto`) → "ver a conta" (`details.proj-conta`, fechado por padrão, com a derivação aritmética). Hierarquia, medida: ordinal < título < número-tema < números da frase-resposta. **Os capítulos 6 e 7 não têm número-tema**, e isso é desenho: o 6 é uma tabela de efeitos (cinco linhas com o mesmo peso, nenhuma é "o" número do capítulo) e o 7 é qualitativo (o que a conta não inclui não tem valor). Pôr um número ali seria inventar destaque.
- **Régua (`.proj-regua`, capítulo 3).** As quatro taxas reais num eixo horizontal. O domínio é **derivado dos dados** (`projReguaDominio`: mínimo e máximo das quatro com margem) e **sempre inclui o 0%**, marcado como referência (`.proj-regua__zero`): uma âncora pode ficar negativa, e a régua mostra isso à esquerda do zero em vez de cortar. Âncoras em traço cheio, mercado em traço fino. **Um rótulo por linha**, nunca dois na mesma altura: a 320 px as taxas podem ficar a menos de meio ponto, e empilhar é o que garante que nenhum rótulo cubra outro (teste de caixas a 320 px). Os rótulos ficam dentro do container da régua.
- **Mapa (`#chart-projecao-mapa`, capítulo 4).** As quatro **medianas** (mercado simples usa a linha `deterministico`, que não tem sorteio). Âncoras em linha cheia (`--ink` e `--g-700`, 2,5 px), mercado tracejado em `--gray` com **dois padrões distintos** (6/4 e 2/3). **Rótulo no fim da linha em vez de legenda** (`endLabel`, o nome em duas linhas vindo da fonte única `projNome`, `labelLayout.moveOverlap`), sem caixa de legenda (anti-pattern #22) e sem dataZoom. O eixo Y tem o rótulo **por dentro** da área, sobre a própria linha de grade, e `grid.right` 92: medido na verificação visual, com o eixo por fora a área de desenho tinha 44 px a 320 e o eixo X perdia "hoje" e "65"; com o eixo por dentro ela passa a 111 px (181 a 390), sem cortar o nome mais longo. Embaixo, a tabela taxa → valor aos 65, na ordem canônica.
- **Barra de decomposição (`.proj-decomp`, capítulo 4, uma por âncora).** Barra 100% de três segmentos (patrimônio de hoje crescido `--ink` · aportes somados `--g-700` · rendimento dos aportes `--gray`) **mais** a lista com os três valores escritos: forma + texto, nunca cor sozinha. **Só parcelas positivas entram na barra**; uma parcela negativa sai da barra e fica só no texto, com o sinal. O rendimento é `--gray`, não `--neutral-300`: no Plantão o `--neutral-300` sumia contra o trilho (`--neutral-200`), medido na verificação visual.
- **Leques em pequenos múltiplos (`.proj-leque`, capítulo 5).** Um por âncora, um sobre o outro, cada um com o próprio título nomeando o cenário (sem ele, dois leques na mesma escala parecem o mesmo gráfico duplicado). **Mesma escala Y** nos dois (`projLequeMax`, `max` explícito), senão a comparação de largura engana. Faixas p10–p90 e p25–p75 planas (exceção nominal ao #17), mediana sólida com `criarMarkPointUltimo`. Só as âncoras têm leque: quatro faixas juntas não se leem.
- **Ponteiro (`.proj-tabela--ponteiro`, capítulo 6).** Uma coluna por âncora; efeito com seta ▲/▼ + cor + sinal. Até 360 px a seta sobe para a linha de cima do valor: as duas colunas com seta e valor lado a lado não cabiam a 320 px.
- **Largura.** `.proj-ensaio` é grid com `grid-template-columns: minmax(0, 1fr)`: com a coluna implícita `auto`, o min-content de qualquer filho (a tabela ano a ano do grifo) alargava o ensaio inteiro além da tela. Valor em prosa numa célula numérica ("antes da medição") quebra linha (`.proj-num--texto`); valor dentro de frase não parte entre número e unidade. O teste de 320 px confere a borda do ensaio, não só a rolagem da página: uma tabela larga demais sobra para dentro do respiro lateral sem gerar rolagem.
...
- **Estados.** Cenário ausente sai da régua, do mapa, dos leques e do ponteiro, e o capítulo 3 diz "indisponível hoje" no lugar dele. `projecao === null` ou payload anterior à v2.28: o card da home some e a rota diz "Projeção indisponível hoje".
- **Plantão.** Os gráficos leem os tokens do tema no boot (`dc.tokens`), então o mapa usa o `--ink` escuro (teste). Verificado visualmente a 390 e 320 px, claro e Plantão, com a fixture completa e com a de âncora ausente.
